...
{
    public class CreateRoomTypeIntegrationTest
    {
        ...
        public CreateRoomTypeIntegrationTest()
        {
            ...
            _service = new RoomTypeService(_unitOfWorkMock.Object, _repoMock.Object);
            _controller = new RoomTypeController(_service, _userServiceMock.Object);
        }

        [Fact]
        public async Task CreateRoomType_Returns_CreatedResult_ControllerService()
        {
            //Arrage
        ...
        [Fact]
        public async Task CreateRoomType_Returns_CreatedResult_ServiceDomain()
        {
            //Arrage
            var dtoMock = new RoomTypeDto("BLCOP-T1", "Bloco Operatório", "Does the surgeries.");
            var roomtype = new Mock<RoomType>("BLCOP-T1", "Bloco Operatório", "Does the surgeries.");
            ...
            Assert.Equal(result.Code, dtoMock.Code);
            Assert.Equal(result.Description, dtoMock.Description);
            Assert.Equal(result.Designation, dtoMock.Designation);
            _repoMock.Verify(r => r.AddAsync(It.IsAny<RoomType>()), Times.Once);
